...
### Step 1: 摸清底細 (背景探索)
1. **優先呼叫 `job104_check_session` 工具**，確認人類的 104 登入狀態是否有效。
   - 如果回傳 `logged_in: false`，立刻停下來告訴人類：「我發現你的 104 登入失效了，請先在終端機執行 `npx tsx src/login.ts` 重新登入，登入完成後再告訴我！」然後等待人類確認，**不要繼續執行後續步驟。**
   - 如果回傳 `logged_in: true`，繼續往下走。
2. 請人類提供**最新履歷、夢想職缺條件，或是簡單的技能樹**。
3. 如果他們說資料存在知識庫（例如 LLM Wiki），就自己用工具去爬出來。
4. 把他們的專案經驗和核心技能稍微在腦袋裡整理一下。
...
3. **對話指示**：表格列完後，停下來問人類：
   > 「這幾家看起來都不錯。你對哪一家比較有興趣？還是需要我先把這幾家加入你的『104 收藏名單 (使用 job104_save_job)』讓你慢慢看？」

### Step 3.5: 深入企業調研 (可選)
- 若人類對某間公司特別感興趣（如台積電、新創），你可以主動提議使用 `job104_get_company_detail` 工具為他進行深度調研，這能幫他抓出該公司的福利、產品，以及「目前所有開放中的其他職缺」，或許能挖出更適合他的隱藏版工作！
